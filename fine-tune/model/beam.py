...
class Beam(object):
    ...
    def advance(self, word_probs, attn_out, hidden):
        """
        ...
        * `attn_out`- attention at the last step
        ...
        self.hidden.append(hidden.index_select(0, prev_k))
    ...
    def get_hyp(self, timestep, k):
        ...
        for j in range(len(self.prev_ks[:timestep]) - 1, -1, -1):
            ...
            hidden.append(self.hidden[j][k])
            k = self.prev_ks[j][k]
        return torch.stack(hyp[::-1]), torch.stack(attn[::-1]), torch.stack(hidden[::-1])
...
class GNMTGlobalScorer(object):
    ...
    def __init__(self, opt=None):
        self.alpha = 0.
        self.beta = 0.
        penalty_builder = PenaltyBuilder('none',
                                        'avg')
