...
parser.add_argument('-c', '--checkpoint_path', type=str,
                        default='emoVC_for_0011/checkpoint_2299',
...
def gen_embedding(speaker):
    ...
    if not os.path.exists(os.path.dirname(checkpoint_path)+'/embeddings'):
        os.makedirs(os.path.dirname(checkpoint_path)+'/embeddings')
    
    np.save(os.path.dirname(checkpoint_path)+'/embeddings/%s.npy'%speaker, speaker_embeddings)
    # plot_data([speaker_embeddings], 
    #     'outdir/embeddings/%s.pdf'%speaker)

for emo in ['Angry','Happy','Sad','Neutral','Surprise']:
    print('Generating embedding of %s ...'%emo)
    gen_embedding(emo)
...
# print('Generating embedding of %s ...'%hparams.speaker_C)
# gen_embedding(hparams.speaker_C)
